...
- 解答可能な全60ステージ、6ワールド、ロック解除とベスト手数
...
Garden のチュートリアル後は、交互色・分断色の盤面へ移行します。Stage 14 以降は空チューブを1本に制限し、6色から8色へ段階的に増やして、一時退避と取り出し順の計画を必要とする固定盤面にしています。移動先が空の場合、または移動元と移動先の最上段が同じ色で、移動先が満杯でない場合だけ移動できます。異なる色の上には置けません。全60ステージには、この1手で1個だけ移動するルールで生成した開発用ソルバーの解法証明を保持し、テストで全手を本番の合法手判定によりリプレイして解答可能性を検証します。Hint は詰み判定と同じ本番の合法手判定・1個移動を使う幅優先探索から、最短クリア距離を必ず1減らす手だけを提示します。同距離では移動元・移動先の番号順で決定し、探索が完了しない場合は安全のため代替手を表示しません。
...
鍵・指定ゴール・連鎖アンロックは通常操作、Undo、Restart、詰み判定、Hintで同じ本番ルールを共有します。連鎖中は各ロックに必要色と段階を表示し、解放進行もUndoで正確に戻ります。Level 41–45は鍵の解放前に使える空きチューブだけでは解けず、解放したチューブの使用が必須です。Level 51–55とLevel 56–60の最短手数は本番Hintと同じ幅優先探索で証明しています。

## Development analysis

依存パッケージなしで、オリジナル30面の詳細解析と全60面の検証を実行できます。
