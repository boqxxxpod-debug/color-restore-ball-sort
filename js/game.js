(function(){
  ...
  function move(tubes,from,to,cap){if(!legal(tubes,from,to,cap))return 0;var count=Math.min(topRun(tubes[from]),cap-tubes[to].length),balls=tubes[from].splice(tubes[from].length-count,count);Array.prototype.push.apply(tubes[to],balls);return count;}
